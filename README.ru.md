...
- **Античиты уровня ядра блокируют утилиту.** FACEIT, Vanguard, EAC и подобные отключают низкоуровневые клавиатурные хуки, на которых работает AutoHotkey, — внутри таких игр ремапы просто перестают действовать (везде остальном продолжают). Программного обходного пути нет: если это про тебя, перенеси ремапы в саму клавиатуру — та же раскладка в виде QMK/VIA-конфига лежит в [`hardware/keychron-k10-pro`](hardware/keychron-k10-pro).
